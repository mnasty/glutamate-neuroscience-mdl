...
| **Lorazepam** | GABA-A receptors on soma & dendritic inhibitory synapses | 💙 Blue (shared w/ GABA) | Benzodiazepine — positive allosteric modulator of GABA-A. Doesn't replace GABA but amplifies GABA's effect, effectively holding the door open wider and longer for $Cl^-$ ions to provide electrical ground, redirecting excess energy safely. Glutumate must work harder to excite neurons because resting membrane potential $(V_m)$ is reduced as a result. Tolerance develops via receptor downregulation, reducing the possible surface area on the soma that GABA can bind to. |
